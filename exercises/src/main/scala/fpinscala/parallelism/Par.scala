...
//  def map2b[A,B,C](a: Par[A], b: Par[B])(f: (A,B) => C): Par[C] = // `map2` doesn't evaluate the call to `f` in a separate logical thread, in accord with our design choice of having `fork` be the sole function in the API for controlling parallelism. We can always do `fork(map2(a,b)(f))` if we want the evaluation of `f` to occur in a separate thread.
//    (es: ExecutorService) => {
//      fork(unit(f(run(es)(a), run(es)(b))))(es) // This implementation of `map2` does _not_ respect timeouts, and eagerly waits for the returned futures. This means that even if you have passed in "forked" arguments, using this map2 on them will make them wait. It simply passes the `ExecutorService` on to both `Par` values, waits for the results of the Futures `af` and `bf`, applies `f` to them, and wraps them in a `UnitFuture`. In order to respect timeouts, we'd need a new `Future` implementation that records the amount of time spent evaluating `af`, then subtracts that time from the available time allocated for evaluating `bf`.
//    }
  ...
  def map2WithTimeout[A,B,C](a: Par[A], b: Par[B])(timeout: Long, units: TimeUnit)(f: (A,B) => C): Par[C] = {
    (es: ExecutorService) => {
      val af = a(es)
      val bf = b(es)
      val maxNanoSeconds = units.toNanos(timeout)
      UnitFuture({
        val start = System.nanoTime()
        val a = af.get(timeout, units)
        val remainingTimeout = units.convert(maxNanoSeconds - (System.nanoTime() - start),TimeUnit.NANOSECONDS)
        val b = bf.get(remainingTimeout, units)
        f(a,b)
  ...
  def sequence[A](as: List[Par[A]]): Par[List[A]] = {
   as.foldLeft(unit[List[A]](Nil)){
     case (acc, a) => map2(acc, a)(_ :+ _)
   }
  }
  ...
  def choiceViaN[A](cond: Par[Boolean])(t: Par[A], f: Par[A]): Par[A] = {
    val n: Par[Int] = map(cond)(c => if (c) 1 else 0)
    choiceN(n)(List(f, t))
  }
  ...
  def choiseViaChooser[A](cond: Par[Boolean])(t: Par[A], f: Par[A]): Par[A] =
